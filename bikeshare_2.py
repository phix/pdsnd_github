...
def get_filters():
    ...
    while True:
        city = input("Pick a City, Chicago, New York City or Washington:")
        city = city.lower()
    ...
    while True:
        month = input("Pick a Month, January, February, All:")
        month = month.lower()
    ...
    while True:
        day = input("Pick a Day of the Week, Monday, Tuesday, All etc:")
        day = day.lower()
...
def time_stats(df):
    ...
    print("Most popular Day: " + str(popular_month))
...
def station_stats(df):
    ...
    popular_trip_station = (df['Start Station'] + df['End Station']).mode()[0]
    print("Most Popular Start and End Station Combination: " +
          popular_trip_station)
    print("\nThis took %s seconds." % (time.time() - start_time))
    print('-'*40)
...
def trip_duration_stats(df):
    ...
    mean_travel_time = format(datetime.timedelta(seconds=mean_travel_time))
...
def user_stats(df):
    ...
    if 'Birth Year' in df.columns:
        ...
        earliest_birth = df['Birth Year'].dt.year.min()
        print("Earliest Birth : %s " % earliest_birth)
        most_recent_birth = df['Birth Year'].dt.year.max()
        print("Most Recent Birth : %s " % most_recent_birth)
        common_birth_year = df['Birth Year'].dt.year.mode()[0]
        print("Most Common Year of Birth : %s " % common_birth_year)
...
def raw(df):
    while True:
        printRaw = input("Would you like to see Raw data , type Y or N:")
        printRaw = printRaw.lower()
        if printRaw not in ('y', 'n'):
            print("Not an appropriate choice.")
        else:
            if printRaw != 'n':
                first = 0
